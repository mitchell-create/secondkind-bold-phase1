# SecondKind Bold, Creative Context Pack (complete)
...
1. **Unspoken-Truths Bank, numbered hook index** (Hook 001 to 031), right below. Every customer-facing line starts here.
2. **Brand essentials, brand context, the position, voice rules, receipts, prohibited terms, headline patterns, sample copy, pre-flight checklist.**
3. **The six personas**, each with a full psychology profile.
4. **Product** (Gut Balance), **offers**, **competitive landscape** + per-brand analyses, **exploitable gaps.**
5. **The Creative Matrix** (pain vs competitor, what they love, wishes & gaps, hooks & angles) and the **Messaging Matrix** (30-cell persona × awareness grid) + cross-stage observations.
6. **Brief summaries** and the **creative-strategy synthesis** (mental stages, 9 psychology heuristics, the four biggest levers, rules of engagement).
7. **Voice-of-customer corpus, reference-ad archetype library, trending-format library**, and the step-by-step build method.

## ICP priority (canonical, use this order)

The primary ICP is **Done-Everything Danielle**. Full priority order, from the persona index:

1. **Done-Everything Danielle** — primary. The center-of-mass persona for the bold variant: clean-living, probiotic-fatigued, vindication-led. Problem-aware.
2. **Immune-Anxious Isaac** — secondary. Performance-coded; converts on the EpiCor 17% RCT. Problem-aware.
3. **New-Normal New Mom Natalie** — tertiary. Postpartum; body is sacred ground (function language only). Problem-aware.
4. **Functional-Curious Practitioner Paul** — quaternary. A separate practitioner channel, peer-to-peer only. Solution-aware.
5. **Burnout Biohacker Brandon** — quinary. Optimizer; mechanism-first, evidence-dense. Problem-aware.
6. **Perimenopause Paula** — senary. Skeptic; gut-hormone reframe with citations. Problem-aware.

Where any section below labels priority differently, **this order governs** (some lower sections were generated from an earlier, incorrect ranking).

## The one rule for everything you make

Open every customer-facing line from the **Unspoken-Truths Bank** below, in the confessional register, never with a generic claim. Then run the four-beat arc: open on the truth, diagnose the mechanism, vindicate the customer ("you weren't wrong, the product was"), close on the receipt and the offer. Honor each persona's register notes (body and appearance language is off-limits for Natalie, Paul, and Isaac), and never name a competitor brand in customer-facing copy. The full rules are in the Voice rules and Operating rules sections below.

---

## Unspoken-Truths Bank, numbered hook index

Each line is a ready hook. Open from one of these (or write a new line in the exact same register). Full persona context for every line is in the **Personas** section further down.

### Done-Everything Danielle (Hooks 001 to 006)
...
### Immune-Anxious Isaac (Hooks 007 to 011)
...
### New-Normal New Mom Natalie (Hooks 012 to 016)
...
### Functional-Curious Practitioner Paul (Hooks 017 to 020)
...
### Burnout Biohacker Brandon (Hooks 021 to 025)
...
### Perimenopause Paula (Hooks 026 to 031)
...
- Sees the brand's founder story in a reel or Story and immediately recognizes that
...
### Burnout Biohacker Brandon (quinary)
...
competitors:
 ...
 tiktok_post_urls: []  # third-party creator handles and video URLs removed for public sharing
...
## THE CREATIVE MATRIX (pain vs competitor · what they love · wishes & gaps · hooks & angles)

**This is the Creative Matrix** — the tactical research grid built from competitor reviews and the VoC corpus. Four tabs: **pain_vs_competitor** (each category complaint, our postbiotic advantage, and a positioning angle), **what_they_love** (what customers praise about competitors, to match or neutralize), **wishes_gaps** (unmet desires the category leaves open), and **hook_angles** (ready hook and angle directions). Rows carry verbatim customer language, `format_fit`, a `static_treatment`, the `psychology_trigger`, and `trending_match` / `trending_next` executions. When someone asks for "the creative matrix," this is it — not the persona × awareness grid that follows.
...
## Messaging Matrix (persona × awareness stage)

*This is the **Messaging Matrix**, distinct from the Creative Matrix above. 6 personas × 5 Schwartz awareness stages = 30 messaging cells. Personas are grouped below for reference; the canonical ICP priority is at the top of this document (Danielle is primary), and the grouping order here does not imply ranking.*
...
**Gap to fill:** The founder's origin story, plus the experiences of other moms who went through this, may directly mirror her own, this is the moment where peer recognition and the brand origin story function as the highest-leverage social proof for this persona.

**Hook style:** story-recognition, the founder story is the hook; her experience is the proof

**Example hook:** *"Other moms came to Gut Balance after exactly what you're describing. The bloating that wouldn't resolve. The fog they were told was just tired-mom life. It exists because nothing they tried was addressing the actual mechanism. That's still why we're here."*
...
### 1. Done-Everything Danielle, Primary
...
### 2. Immune-Anxious Isaac, Secondary
...
### 3. New-Normal New Mom Natalie, Tertiary
...
>
> *"Other moms came to Gut Balance after exactly what you're describing. It exists because nothing they tried was addressing the actual mechanism."*
>
...
### 5. Burnout Biohacker Brandon, Quinary
...
- total_reviews: 247
 ...
 - "Multiple comments reference a a named dietitian creator and 'Ritual'\
...
The Messaging Matrix cell already suggests one mechanic + one visual format. You can swap to another if the slot calls for it (e.g., Slot 4 Curiosity Gap pairs naturally with "Talking Head Confession" or "Open Loop B-roll", not with a static product hero). If you have access to the `creative-mechanics` and `visual-formats` skills, consult them. Otherwise, use the mechanic that already showed up in similar briefs in the **Brief summaries** table.
